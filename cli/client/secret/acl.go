...
var (
	ACLTools = tool.NewGroup(
		tool.GroupDef{
			...
			Info:  "Manage secret access controls",
		},
		GrantCommand,
		RevokeCommand,
		RosterCommand,
	...
	GrantCommand = tool.NewCommand(
		tool.CommandDef{
			...
			Exec: func(env tool.Environment, c *cli.Context) (err error) {
				return client.GrantPolicyMember(env, c, "secret")
			},
		})
...
`,
			Exec: func(env tool.Environment, c *cli.Context) (err error) {
				return client.RevokePolicyMember(env, c, "secret")
			},
		})
	...
	RosterCommand = tool.NewCommand(
		tool.CommandDef{
			...
			Help:  ``,
			Exec: func(env tool.Environment, c *cli.Context) (err error) {
				return client.ListPolicyMembers(env, c, "secret")
			},
		})
)
